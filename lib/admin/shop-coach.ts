// The SHOP coach on desktop — the same deterministic reads the app's shop home
// runs (app-native/lib/shop-coach.ts), fed from the admin contexts' shapes.
// Plain English, math on the shop's own rows, no AI, no guessing. If you
// change a read here, change it there — the two voices must never disagree.
...
export function shopCoachTips(opts: {
  ...
  const recent = opts.sales.filter((s) => s.date >= sixtyAgo);
  const recentTotal = recent.reduce((a, s) => a + s.serviceCents + s.tipCents, 0);
  ...
  for (const b of opts.bookings) {
    ...
    if (d >= sixtyAgo && d < tk) seen.add(b.client_id);
  }
  for (const b of opts.bookings) {
    if (!b.client_id || !seen.has(b.client_id)) continue;
    if ((b.starts_at || "").slice(0, 10) >= tk) rebooked.add(b.client_id);
  }
  ...
  for (const s of recent) {
    ...
    byArtist.set(s.artistId, (byArtist.get(s.artistId) ?? 0) + s.serviceCents + s.tipCents);
  }
  ...
  for (const s of recent) {
    if (!s.date) continue;
    const k = new Date(`${s.date}T00:00:00`).getDay();
    const cell = byDow.get(k) ?? { total: 0, days: new Set<string>() };
    cell.total += s.serviceCents + s.tipCents;
    cell.days.add(s.date);
    byDow.set(k, cell);
  }
  ...
  const upcoming = opts.bookings.filter(
    (b) => b.status === "scheduled" && (b.starts_at || "").slice(0, 10) >= tk,
  );
  ...
  const thisWeek = weekKey(tk);
  const byWeek = new Map<string, number>();
  for (const s of opts.sales) {
    if (s.date) byWeek.set(weekKey(s.date), (byWeek.get(weekKey(s.date)) ?? 0) + s.serviceCents + s.tipCents);
  }
  ...
  if (best > 0 && cur > best * 0.6 && cur < best && (dow >= 4 || dow === 0)) {
    tips.push({
      ...
      body: `Best week so far: ${usd(best)}. This week: ${usd(cur)}, with days left. One flash day or a couple of walk-ins closes it.`,
    });
  }
