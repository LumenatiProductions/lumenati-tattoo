...
export default function WeekCalendar({
  ...
  return (
    <div className="rounded-xl border border-black/8 bg-white shadow-sm">
      {/* Week nav */}
      <div className="flex items-center justify-between border-b border-black/8 px-4 py-2.5">
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => setWeekOffset((w) => w - 1)} className="rounded-md border border-black/10 px-2 py-1 text-xs text-black/60 hover:bg-black/4" aria-label="Previous week">‹</button>
          <button onClick={() => setWeekOffset(0)} disabled={weekOffset === 0} className="rounded-md border border-black/10 px-2.5 py-1 text-xs font-medium text-black/60 hover:bg-black/4 disabled:opacity-40">Today</button>
          <button onClick={() => setWeekOffset((w) => w + 1)} className="rounded-md border border-black/10 px-2 py-1 text-xs text-black/60 hover:bg-black/4" aria-label="Next week">›</button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[840px]">
          {/* Day headers */}
          <div className="grid border-b border-black/8" style={{ gridTemplateColumns: "48px repeat(7, 1fr)" }}>
            <div />
            {days.map((d) => {
              const k = dayKey(d);
              return (
                <div key={k} className={`px-2 py-2 text-center ${k === todayK ? "bg-brand-soft" : ""}`}>
                  <div className={`text-[11px] font-medium uppercase tracking-wide ${k === todayK ? "text-brand" : "text-black/40"}`}>
          ...
          <div className="grid" style={{ gridTemplateColumns: "48px repeat(7, 1fr)" }}>
            ...
            <div className="relative" style={{ height: hours.length * HOUR_PX }}>
              {hours.map((h, i) => (
                <div key={h} className="absolute right-1.5 text-[10px] text-black/35" style={{ top: i * HOUR_PX - 6 }}>
                  {i === 0 ? "" : h <= 12 ? `${h}a` : `${h - 12}p`}
                </div>
              ))}
            </div>
            ...
            {days.map((d) => {
              ...
              return (
                <div
                  key={k}
                  className={`relative border-l border-black/6 ${k === todayK ? "bg-brand-soft/40" : ""}`}
                  style={{ height: hours.length * HOUR_PX }}
                >
                  {/* hour lines */}
                  {hours.map((h, i) => (
                    <div key={h} className="absolute inset-x-0 border-t border-black/4" style={{ top: i * HOUR_PX }} />
                  ))}
                  {dayBookings.map((b) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="block truncate text-black/55">{b.service_desc || artist?.name || "Booking"}</span>
                      </button>
                    );
                  })}
                </div>
      ...
      {/* Artist legend */}
      <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-black/8 px-4 py-2">
        {artists.map((a) => (
          <span key={a.id} className="flex items-center gap-1.5 text-[11px] text-black/50">
