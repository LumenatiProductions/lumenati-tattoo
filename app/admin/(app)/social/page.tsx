  ...
  return (
    <div>
      ...
      <Card className="mb-5">
        ...
        {formError && <div className="px-4 pb-3 text-xs text-rose-400">{formError}</div>}
        {error && !formError && <div className="px-4 pb-3 text-xs text-amber-400">{error}</div>}
      </Card>
    ...
    <Card className={`overflow-hidden ${post.featured ? "ring-1 ring-brand/40" : ""}`}>
      ...
      <a href={post.permalink} target="_blank" rel="noreferrer" className="block">
        {post.media_url ? (
          ...
          <div className="flex aspect-square w-full flex-col items-center justify-center gap-1 bg-white/6 text-center">
      ...
      <div className="p-3">
        ...
        {post.caption && (
          <p className="mb-2 line-clamp-2 text-xs text-white/70">{post.caption}</p>
        )}
        ...
        <div className="flex items-center justify-between gap-2">
          <select
            ...
            className="max-w-[55%] rounded-md border border-white/12 bg-white/6 px-1.5 py-1 text-[11px]"
          >
            <option value="">Shop / unattributed</option>
            {artists.map((a) => (
              <option key={a.id} value={a.id}>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="rounded-md border border-white/12 px-2 py-1 text-[11px] text-white/60 hover:bg-rose-400/10 hover:text-rose-400"
            >
              Remove
            </button>
          </div>
