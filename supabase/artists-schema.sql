...
create table if not exists public.artists (
  ...
  pay_type    text not null default 'split' check (pay_type in ('rent','split','hybrid')),
  rent_cents  int not null default 0,
  split_pct   numeric not null default 0,  -- 0..1, shop's cut
...
insert into public.artists (id, slug, name, handle, color, pay_type, rent_cents, split_pct, guest, active, room_extras, sort) values
  ('jd',      'jd-pruitt',        'J.D. Pruitt',      'jd.pruitt',       '#FF1493', 'split',  0,      0.35, false, true, true,  1),
  ('elaine',  'electric-elaine',  'Electric Elaine',  'electric.elaine', '#FFD700', 'rent',   120000, 0,    false, true, false, 2),
  ('shorty',  'shorty',           'ShorTy',           'shorty.tattoo',   '#7FFF00', 'hybrid', 60000,  0.15, false, true, false, 3),
  ('kalypso', 'king-kalypso',     'King Kalypso',     'king.kalypso',    '#1493FF', 'split',  0,      0.30, false, true, false, 4),
  ('sam',     'sam-durbin-clark', 'Sam Durbin-Clark', 'sam.durbinclark', '#9b59b6', 'rent',   100000, 0,    false, true, false, 5),
  ('moonie',  'moonie-b-jones',   'Moonie B. Jones',  'moonie.b.jones',  '#FF6347', 'split',  0,      0.40, true,  true, false, 6)
